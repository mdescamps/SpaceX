...
public class Planet_adv implements Serializable {
	 ...
	 */
	public Planet_adv(int rate, int player, Sprite_adv sprite) {
		this.productionRate = rate;
		this.player = player;
		this.spaceShips = new ArrayList<SpaceShip_adv>();
		this.nbSpaceShips = this.spaceShips.size();
		this.sprite = sprite;
		Point2D_adv center = new Point2D_adv(sprite.getX() + (sprite.width()/2) - 10, sprite.getY() + (sprite.height()/2) - 10);
		this.c = new Circle_adv(center,(sprite.height() / 2));
		this.selected = false;
		this.Shield = 0;
	 ...
	 */
	public void setSprite(Sprite_adv s) {
		this.sprite = s;
	}
